...
__copyright__ = "Copyright 2022, Laboratory of Human Genetics of Infectious Diseases, The Rockefeller University"
...
print('    #   #   #   #   #   #       #   #   #   #       #        ')
print('     ###    #   #    ###    ####     ###    #####   #####    ')
print('Genotyping MUltiplexed-Sequencing of CRISPR-Localized Editing')
print('*************************************************************\n')
...
parser.add_argument("--readtype", type=str, default='se', choices=['se','pe'], help="single-end or paired-end [default: se]")
...
sample_list = list()
for eachsample in file_sample_list:
    ...
    # load raw reads
    ###
    file_fasta = open('xtemp_' + sample + '_reads.fasta', 'w')
    print('> Sample:', sample)
    total_read_count = 0
    ...
    file_summary.write('# total / unique reads (raw):\t' + str(total_read_count) + ' / ' + str(total_read_count_unique) + '\n')
    file_summary.write('# total / unique reads (pass):\t' + str(passed_read_count) + ' / ' + str(passed_read_count_unique) + '\n')
    file_fastq.close()
    file_fasta.close()
    ...
    ###
    command_blast_reads_to_ref_seq = 'blastn -query xtemp_' + sample + '_reads.fasta -db ref_seq.fasta '\
                                     '-reward 5 -penalty -4 -gapopen 10 -gapextend 6 ' \
                                     '-outfmt \"6 qseqid gaps mismatch length qstart qend qseq sstart send sseq\" ' \
                                     '-out xtemp_' + sample + '_reads_blast.txt'
    ...
    ###
    file_blast = open('xtemp_' + sample + '_reads_blast.txt', 'r')
    wt_read_count = 0
    ...
    for eachline in file_blast:
        ...
        if gap_size == 0 and mismatch_size == 0:
            ...
            try:
                ...
                for i in range(0, length):
                    if read_alignment[i] != ref_alignment[i]:
                        ...
                        if (i >= flag + gap_size + mismatch_size) and \
                           (read_alignment[i+1] == ref_alignment[i+1]):
                            break
            except IndexError:
                pass
            ...
            if len(REF) == 1 and len(ALT) == 1:
                ...
                muttype = 'del-' + str(len(REF)-len(ALT)) + 'nt'
            elif len(REF) < len(ALT):
                muttype = 'ins-' + str(len(ALT)-len(REF)) + 'nt'
    ...
    ###
    file_plot = open('xtemp_' + sample + '_plot.txt', 'w')
    file_plot.write('POS\tINDEL\tSIZE\tRATIO\n')
    ...
    file_summary.write('# wild-type reads:\t' + str(wt_read_count) + '\n')
    file_summary.write('# genotypes:\t' + str(genotype_index) + '\n')
    file_summary.write(genotype_output + '\n\n')
    file_plot.close()
    os.system('rm xtemp_' + sample + '_reads.fasta')
    os.system('rm xtemp_' + sample + '_reads_blast.txt')
...
###
POS_min = min(global_genotype_pos_list)-1
POS_max = max(global_genotype_pos_list)+1
ratio_max = 100*max(global_genotype_ratio_list)+5
for each_sample in sample_list:
    filename_plot_in = 'xtemp_' + each_sample + '_plot.txt'
    ...
    + geom_bar(aes(x='POS', y='RATIO', fill='INDEL'), width=0.3, stat='identity', color='darkgrey', size=0.3)
    + geom_text(aes(x='POS', y='RATIO', label='SIZE'), size=9, angle=22, nudge_y=2)
    + scale_fill_manual({'del':'#4D6FE6', 'ins':'#E8C943'})
    + lims(x=(POS_min, POS_max), y=(0, ratio_max))
    + labs(title='Sample: '+each_sample+'', x='Genomic Position ('+CHR+')', y='Proportion (%)', fill='Indel (nt)')
    + theme_538()
